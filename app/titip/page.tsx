...
Cek detail: ${linkDetail}`
  ...
  const getSlotClassName = (slotNumber: number): string => {
    ...
    const baseClasses =
      "w-16 h-16 rounded-lg font-bold text-sm transition-all duration-200 flex items-center justify-center shadow-md";
    ...
    switch (status) {
      ...
      default:
        return `${baseClasses} bg-card border-2 border-input text-foreground hover:border-blue-500 hover:scale-105 cursor-pointer dark:hover:border-blue-400`;
    }
  };
    ...
    <div className="min-h-screen bg-background text-foreground py-8 px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <div className="bg-card rounded-2xl shadow-xl border border-border overflow-hidden">
              <div className="bg-gradient-to-r from-purple-500 to-indigo-600 px-6 sm:px-8 py-6">
              ...
              <div className="p-4 sm:p-6 md:p-8">
                {/* Legend */}
                <div className="flex flex-wrap gap-3 sm:gap-4 mb-6 pb-6 border-b border-border">
                  <div className="flex items-center space-x-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-card border-2 border-input rounded-lg"></div>
                  ...
                  <div className="flex items-center space-x-2">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-300 dark:bg-gray-700 rounded-lg"></div>
                    <span className="text-xs sm:text-sm text-muted-foreground font-medium">
                      Terisi
                    </span>
                  </div>
                ...
                {slot && (
                  <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      <strong>Slot terpilih:</strong>{" "}
                      <span className="text-2xl font-bold ml-2">{slot}</span>
                    </p>
                  </div>
                )}
                ...
                >
                  ...
                  <div className="space-y-1 sm:space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-input rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none bg-background text-foreground placeholder:text-muted-foreground text-sm sm:text-base"
                      required
                    />
                  </div>
                    ...
                    {fotoUrl && (
                      <div className="mt-4 relative group">
                        <img
                          src={fotoUrl}
                          alt="preview"
                          className="w-full rounded-xl shadow-md border-2 border-border"
                        />
                        <button
                          type="button"
                          onClick={() => setFotoUrl(undefined)}
                          className="absolute top-2 right-2 bg-destructive hover:bg-destructive/90 text-destructive-foreground p-1 sm:p-2 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          <svg
                  ...
                  <div className="pt-4">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-muted disabled:to-muted text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-xl shadow-lg hover:shadow-xl transform transition-all duration-200 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center space-x-2 sm:space-x-3 text-sm sm:text-base"
                    >
                      {loading ? (
                        <>
                          <svg
                    ...
                    {!slot && (
                      <p className="text-center text-xs sm:text-sm text-destructive mt-2 font-medium">
      ...
      {showSuccessNotif && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-card rounded-2xl sm:rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-scaleIn">
            <div className="relative bg-gradient-to-r from-green-500 to-emerald-600 px-6 sm:px-8 py-6">
              <button
                onClick={handleCloseNotification}
                className="absolute top-3 right-3 sm:top-4 sm:right-4 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-white/20 hover:bg-white/30 rounded-full transition-colors duration-200"
                aria-label="Close"
              >
                <svg
            ...
            <div className="p-4 sm:p-6 md:p-8">
              <div className="text-center mb-4 sm:mb-6">
                ...
                <p className="text-muted-foreground text-xs sm:text-sm">
                  Notifikasi telah dikirim via WhatsApp
                </p>
              </div>

              <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                <div className="flex items-center justify-between p-3 sm:p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-800">
                ...
                <div className="flex items-center justify-between p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                  <span className="text-sm font-medium text-muted-foreground">
                    Kode Ambil:
                  </span>
                  <span className="text-lg sm:text-xl font-bold text-blue-600 dark:text-blue-400 tracking-wider">
                    {successData.kode}
                  </span>
                </div>
              </div>
              ...
              <button
                onClick={handleCloseNotification}
                className="w-full py-3 sm:py-4 px-4 sm:px-6 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200 text-sm sm:text-base"
              >
